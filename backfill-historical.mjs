/**
 * Backfill historical prices: fetches data from each stock's last_date+1 to today.
 * Uses the Tadawul API directly via Puppeteer session.
 */
...
async function main() {
  ...
  // Get last_date for each symbol
  const result = await pool.query('SELECT symbol, MAX(date) as last_date FROM "HistoricalPrice" GROUP BY symbol');
  const lastDates = {};
  for (const row of result.rows) lastDates[row.symbol] = row.last_date;

  // Today's date in DD-MM-YYYY (Riyadh)
  ...
  console.log('API URL captured.');
  console.log('Today (Riyadh):', todayStr);

  const allSymbols = Object.values(SECTORS).flat();
  let completed = 0, totalAdded = 0, failed = 0;
  const startTime = Date.now();

  for (const symbol of allSymbols) {
    completed++;
    const lastDate = lastDates[symbol];
    if (!lastDate) { console.log(`[${completed}/${allSymbols.length}] ${symbol}: NO DATA — skip`); continue; }

    // Calculate start date = last_date + 1 day
    const lastD = new Date(lastDate);
    lastD.setDate(lastD.getDate() + 1);
    const startStr = lastD.toISOString().split('T')[0];

    if (startStr > todayStr) {
      console.log(`[${completed}/${allSymbols.length}] ${symbol}: up to date`);
      continue;
    }

    process.stdout.write(`[${completed}/${allSymbols.length}] ${symbol} (${startStr} → ${todayStr})... `);
    ...
    try {
      ...
      while (true) {
        const result = await page.evaluate(async (url, startIdx, sym, sector, sd, ed) => {
        ...
        }, apiUrl, start, symbol, sectorCode, startDdmmyyyy, todayDdmmyyyy);
        ...
        added += await batchInsert(pool, batch);
        start += records.length;
        if (start >= (result.recordsFiltered || 0)) break;
  ...
  await browser.close();
  await pool.end();
  console.log(`\n=== DONE === +${totalAdded} new records, ${completed - failed}/${allSymbols.length} stocks, ${((Date.now()-startTime)/1000/60).toFixed(1)} min`);
}
